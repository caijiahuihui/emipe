...
/**
 * @CacheConfig：
 * 主要用于配置该类中会用到的一些共用的缓存配置。在这里@CacheConfig(cacheNames = "user")：
 * 配置了该数据访问对象中返回的内容将存储于名为student的缓存对象中，我们也可以不使用该注解，
...
public class UserService {
     ...
     * @Cacheable： 配置了findById函数的返回值将被加入缓存
     ...
     * 若自己配置需使用SpEL表达式，比如：@Cacheable(key = "#p0")：使用方法的第一个参数作为缓存的key值，
